...
// ExperienceHandler handles HTTP requests for the Experience entity.
type ExperienceHandler struct {
	useCase input.ExperienceUseCase
}

// NewExperienceHandler creates a new ExperienceHandler.
func NewExperienceHandler(useCase input.ExperienceUseCase) *ExperienceHandler {
	return &ExperienceHandler{useCase: useCase}
}

// List handles GET /v1/ms-resume/experiences
func (h *ExperienceHandler) List(w http.ResponseWriter, r *http.Request) {
	data, err := h.useCase.List(r.Context())
	if err != nil {
		WriteError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to list experiences")
		return
	}
...
// GetByID handles GET /v1/ms-resume/experiences/{id}
func (h *ExperienceHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, entity.ErrNotFound) {
			WriteError(w, r, http.StatusNotFound, "NOT_FOUND", "Experience not found")
			return
		}
...
// Create handles POST /v1/ms-resume/experiences
func (h *ExperienceHandler) Create(w http.ResponseWriter, r *http.Request) {
	var req request.ExperienceRequest
	if !decodeAndValidate(w, r, &req) {
		return
	}
	resp, err := h.useCase.Create(r.Context(), &req)
	if err != nil {
		WriteError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to create experience")
		return
	}
	w.Header().Set("Content-Type", "application/json")
...
// Update handles PUT /v1/ms-resume/experiences/{id}
func (h *ExperienceHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.useCase.Update(r.Context(), id, &req); err != nil {
		...
		WriteError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to update experience")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}

// Delete handles DELETE /v1/ms-resume/experiences/{id}
func (h *ExperienceHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.useCase.Delete(r.Context(), id); err != nil {
		...
		WriteError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to delete experience")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
